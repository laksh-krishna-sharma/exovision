...
from app.models.prediction import PredictionRequest, PredictionResponse, PredictionRecord
...
def find_model_file() -> str:
    ...
    possible_paths = [
        ...
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "../../models/kepler_ann.keras"),
    ...
    raise FileNotFoundError(f"Model file 'kepler_ann.keras' not found in any of the expected locations: {possible_paths}")
...
class PredictionService:
    ...
    def load_model(self):
        ...
        if self.model is None:
            if tf is None:
                raise ImportError("TensorFlow is not installed. Please install it to use predictions.")
            ...
            if not os.path.exists(self.model_path):
                ...
                for alt_path in alternative_paths:
                    ...
                    raise FileNotFoundError(f"Model file not found at {self.model_path} or any alternative paths")
    ...
    async def predict(self, data: PredictionRequest, db: AsyncSession, user_id: Optional[int] = None) -> PredictionResponse:
        ...
        try:
            ...
            log.info(f"Prediction made: ID={prediction_id}, Result={prediction}, Confidence={confidence}")
    ...
    async def get_predictions(
        ...
        try:
            ...
            query = query.offset(skip).limit(limit).order_by(PredictionRecord.created_at.desc())
    ...
    async def delete_prediction(self, db: AsyncSession, prediction_id: str, user_id: Optional[int] = None) -> bool:
