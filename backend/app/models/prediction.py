...
class PredictionRequest(BaseModel):
    ...
    koi_fpflag_ec: float = Field(
        ..., description="Ephemeris Match Indicates Contamination Flag"
    )
    koi_period: float = Field(..., description="Orbital Period [days]")
    koi_period_err1: float = Field(
        ..., description="Orbital Period Upper Uncertainty [days]"
    )
    koi_period_err2: float = Field(
        ..., description="Orbital Period Lower Uncertainty [days]"
    )
    koi_time0bk: float = Field(..., description="Transit Epoch [BKJD]")
    koi_time0bk_err1: float = Field(
        ..., description="Transit Epoch Upper Uncertainty [BKJD]"
    )
    koi_time0bk_err2: float = Field(
        ..., description="Transit Epoch Lower Uncertainty [BKJD]"
    )
    ...
    koi_duration_err1: float = Field(
        ..., description="Transit Duration Upper Uncertainty [hrs]"
    )
    koi_duration_err2: float = Field(
        ..., description="Transit Duration Lower Uncertainty [hrs]"
    )
    koi_depth: float = Field(..., description="Transit Depth [ppm]")
    koi_depth_err1: float = Field(
        ..., description="Transit Depth Upper Uncertainty [ppm]"
    )
    koi_depth_err2: float = Field(
        ..., description="Transit Depth Lower Uncertainty [ppm]"
    )
    koi_prad: float = Field(..., description="Planetary Radius [Earth radii]")
    koi_prad_err1: float = Field(
        ..., description="Planetary Radius Upper Uncertainty [Earth radii]"
    )
    koi_prad_err2: float = Field(
        ..., description="Planetary Radius Lower Uncertainty [Earth radii]"
    )
    koi_teq: float = Field(..., description="Equilibrium Temperature [K]")
    koi_teq_err1: float = Field(
        ..., description="Equilibrium Temperature Upper Uncertainty [K]"
    )
    koi_teq_err2: float = Field(
        ..., description="Equilibrium Temperature Lower Uncertainty [K]"
    )
    koi_insol: float = Field(..., description="Insolation Flux [Earth flux]")
    koi_insol_err1: float = Field(
        ..., description="Insolation Flux Upper Uncertainty [Earth flux]"
    )
    koi_insol_err2: float = Field(
        ..., description="Insolation Flux Lower Uncertainty [Earth flux]"
    )
    ...
    koi_steff_err1: float = Field(
        ..., description="Stellar Effective Temperature Upper Uncertainty [K]"
    )
    koi_steff_err2: float = Field(
        ..., description="Stellar Effective Temperature Lower Uncertainty [K]"
    )
    koi_slogg: float = Field(
        ..., description="Stellar Surface Gravity [log10(cm/s**2)]"
    )
    koi_slogg_err1: float = Field(
        ..., description="Stellar Surface Gravity Upper Uncertainty [log10(cm/s**2)]"
    )
    koi_slogg_err2: float = Field(
        ..., description="Stellar Surface Gravity Lower Uncertainty [log10(cm/s**2)]"
    )
    koi_srad: float = Field(..., description="Stellar Radius [Solar radii]")
    koi_srad_err1: float = Field(
        ..., description="Stellar Radius Upper Uncertainty [Solar radii]"
    )
    koi_srad_err2: float = Field(
        ..., description="Stellar Radius Lower Uncertainty [Solar radii]"
    )
...
class PredictionResponse(BaseModel):
    ...
    prediction: int = Field(
        ..., description="Predicted class (0: No exoplanet, 1: Exoplanet)"
    )
